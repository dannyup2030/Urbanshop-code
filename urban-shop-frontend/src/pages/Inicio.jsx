    ...
    <div className="bg-gray-100 min-h-screen flex flex-col">
      ...
      <section className="relative my-10">
        ...
        <div className="relative max-w-7xl mx-auto">
          <button 
            onClick={() => scroll('left')} 
            className="absolute left-2 top-1/2 -translate-y-1/2 z-10 bg-black bg-opacity-50 hover:bg-opacity-80 text-white p-2 rounded-full"
          >
            &#8592;
          </button>
          
          {/* Pasamos el ID al componente Productos */}
          <div id="carousel" className="overflow-x-auto scroll-smooth">
            <Productos products={products} onAddToCart={handleAddToCart} />
          </div>
          
          <button 
            onClick={() => scroll('right')} 
            className="absolute right-2 top-1/2 -translate-y-1/2 z-10 bg-black bg-opacity-50 hover:bg-opacity-80 text-white p-2 rounded-full"
          >
            &#8594;
          </button>
        </div>
      ...
      <footer className="bg-black text-center text-white p-4 mt-auto">
        &copy; 2025 Urban Shop - Danny Urrea.
      </footer>
    </div>
  );
}